...
<article id="post-<?php the_ID(); ?>" <?php post_class($classes); ?>>
	...
	<header class="entry-header">
				...
				<?php if (get_post_format() == 'gallery' && $gallery_slider && !empty($gallery['src'])) : ?>
					<ul class="post-gallery-slider owl-carousel">
						...
						foreach ($gallery['src'] as $src) {
							if (preg_match("/data:image/is", $src)) continue;
						?>
							<li>
		...
		<?php
		if (is_single()) {
			echo do_shortcode("[template base='ads' descr='banner']");
		}
		?>
	</header><!-- .entry-header -->
	...
	<?php endif; ?>
	<?php
	$buy_now_container = get_field('shoplist');
	$buy_now_msg = get_field('shoplist-txt');
	$active = 0;

	if($buy_now_container){
		for ($i = 0; $i < count($buy_now_container); ++$i) {
			if ($buy_now_container[$i]['checkbox']) {
				$active = $active + 1;
			}
		}
	}
	if ($buy_now_container && $active > 0 ) :
	...
	?>
		...
		<div class='c_shopnow__container'>
			...
			if ($buy_now_msg['shoplist-txt-active']) :
			?>
				<div class='c_shopnow__msg'>
					<h2>
						<?php echo $buy_now_msg['shoplist-txt-msg-title']; ?>
					</h2>
					<p>
						<?php echo $buy_now_msg['shoplist-txt-msg']; ?>
					</p>
				</div>
			<?php
			endif;
			...
			<?php
			foreach ($buy_now_container as $shop) :
			?>
				<?php
				if ($shop['checkbox']) :
				?>
					<a class='c_shopnow__link' href="<?php echo $shop['url']; ?>">
						<?php echo wp_get_attachment_image($shop['icon'], array('700', '600'), "", array("class" => "img-responsive"));  ?>
						<?php
						if ($shop['btn-text']) {
							echo $shop['btn-text'];
						}
